...
export default {
  ...
  computed: {
    _seconds: () => 1000, // 1 second is 1000 miliseconds
    _minutes() {
      return this._seconds * 60; // 1 minute is 60x1000 miliseconds
    },
    _hours() {
      return this._minutes * 60;
    },
    _days() {
      return this._hours * 24; // day has 24 hours
    },
    end() {
      return new Date(
        this.year,
  ...
  methods: {
    ...
    showRemaining() {
      const timer = setInterval(() => {
        const now = new Date();
        //const end = new Date(2021, 12, 24, 10, 10, 10, 10);
